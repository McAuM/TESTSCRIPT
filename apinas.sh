...
function listAllFiles() {
	flag=$2	
	total=$(curl -X GET "http://${ip}:${port}/webapi/FileStation/file_share.cgi?api=SYNO.FileStation.List&version=1&method=list&_sid=${sid}&folder_path=${1}" -s | jq '.data.total' )	
	# for (( i=0; i<${total}; i++ ))
	# {
	total=$((total-1))
	for  i in {0..${total}}
	{		
		count=${i}		
		cmd="curl -X GET 'http://${ip}:${port}/webapi/FileStation/file_share.cgi?api=SYNO.FileStation.List&version=1&method=list&_sid=${sid}&folder_path=${1}' -s | jq '.data.files[$i].isdir'" 
		isdir=$(eval $cmd)
		#echo "isdir: ${isdir}"
		if [ "$isdir" = "true" ]
		then				
				cmd="curl -X GET 'http://${ip}:${port}/webapi/FileStation/file_share.cgi?api=SYNO.FileStation.List&version=1&method=list&_sid=${sid}&folder_path=${1}' -s | jq '.data.files[$i].path' " 
				path=$(eval $cmd)
			...
			cmd="curl -X GET 'http://${ip}:${port}/webapi/FileStation/file_share.cgi?api=SYNO.FileStation.List&version=1&method=list&_sid=${sid}&folder_path=${1}' -s | jq '.data.files[$i].name' " 
			name=$(eval $cmd)
